...
object SMemcachedApplication {
    ...
    options.addOption("t", "threads", true, "number of NIO threads, defaults to " + defs("threads"))
    options.addOption("e", "elements", true, "max number of cached elements to hold in memory, defaults to " + defs("maxElements"))
    options.addOption("d", "disk",true,"overflow to disk if max elements is reached, defaults to true")
    options.addOption("ttl","timetolive",true,"default time to live for the cache in seconds, defaults to 3600")
